...
class DetLayer(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        self._anchors = generate_anchors(base_size = cfg.MINANCHOR, scales=np.array([anchor_scale, anchor_scale + 1]), ratios = [0.333, 0.5, 1, 2, 3])
    ...
    def forward(self, bottom, top):
        ...
        _ndim, _tmp, height, width = bottom[0].data.shape
        ...
        K = shifts.shape[0]
        anchors = self._anchors.reshape((1, self._num_anchors, 4)) + \
                  shifts.reshape((1, K, 4)).transpose((1, 0, 2))
        anchors = anchors.reshape((K * self._num_anchors, 4))
        ...
        all_scores_det = bottom[1].data[:, 1:, :, :].reshape(self._ndim, self._numclasses - 1, self._num_anchors, height, width)
        ...
        all_anchors = np.tile(anchors, (self._ndim, 1))
