...
def draw_label(subplot, bboxes, labels, color='cyan', offset=0, fontcolor=None):
    # bbox: xmin, ymin, xmax, ymax
    #
    '''
    ...
    for index, b in enumerate(bboxes):
        ...
        #              alpha=alpha,
        #              bbox={'facecolor': 'white', 'alpha': 0.5, 'pad': log(w + h)})
        # subplot.text(b[0] + fontsize*2, b[3] + fontsize*5*len(labels), '\n'.join([str(label[index]) for label in labels]),
        #              color=fontcolor,
        #              fontsize=fontsize,
        #              alpha=1,
        #              bbox={'facecolor': 'white', 'alpha': 0.5})
        # + h + fontsize*3*len(labels) - offset
        subplot.text(b[0], b[3] + fontsize*(len(labels)+5), '\n'.join([str(label[index]) for label in labels]),
                     color=fontcolor,
                     fontsize=fontsize,
                     alpha=1,
                     bbox={'facecolor': 'white', 'alpha': 0.5})
