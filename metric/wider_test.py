...
caffe_root = '../../caffe-ron/python'  # this file is expected to be in {caffe_root}/sfd_test_code
lib_root = '../../lib'
import os
import sys
...
def initializeNet(gpuid, model_def, model_weights):
    caffe.set_device(gpuid)
    caffe.set_mode_gpu()

    model_def = os.path.join(cfg.ROOT_DIR, model_def)
    model_weights = os.path.join(cfg.ROOT_DIR, model_weights)
...
def run_wider():
    ...
    model_def = '/data6/yyliang/cuda-workspace/py-faster-rcnn_face/SFD/caffe/models/VGGNet/WIDER_FACE/SFD/deploy.prototxt'
    model_weights = '/data6/yyliang/cuda-workspace/py-faster-rcnn_face/SFD/caffe/models/VGGNet/WIDER_FACE/SFD/SFD.caffemodel'
    net = caffe.Net(model_def, model_weights, caffe.TEST)
    ...
    Path = '/data6/yyliang/cuda-workspace/py-faster-rcnn_face/SFD/DB/face/wider/val/'
    ...
    for index, event in enumerate(event_list):
        ...
        for num, file in enumerate(filelist):
            ...
            Image_Path = Path + event[0][0].encode('utf-8') + '/' + im_name[:] + '.jpg'
...
def wider_test(dbtype):
    gpuid = 1
    model_def = 'models/wider/VGG16/test320cudnn.prototxt'
    model_weights = 'output/wider/RON__iter_50000.caffemodel'
    output_dir = get_output_dir('test/wider')
    net = initializeNet(gpuid, model_def, model_weights)
    suffix = '_c8_n3_s640'  # _c01_n4(320)
    net.name = os.path.splitext(os.path.basename(str(model_weights)))[0] + suffix
    cfg.GPU_ID = gpuid
    cfg.TEST.DET_MIN_PROB = 0.8  # 0.01
    cfg.TEST.NMS = 0.3  # 0.4
    cfg.TEST.SCALES = (640,)
    if dbtype == 'val':
        os.chdir('../')
        imdb = get_imdb('wider_val')
        metric_str = test_net_det(net, imdb, output_dir, recompute=0, vis=1)
        print metric_str[-1]
    resultPath = os.path.join(output_dir, 'Metric_wider.txt')
