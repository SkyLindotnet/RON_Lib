...
class AnchorTargetLayer(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        if cfg.TRAIN.NOBORDER:
            self._allowed_border = 9999.0
        else:
            self._allowed_border = float(border_str)

        if cfg.TRAIN.SET_RPN_EACH_BATCH:
            self._batch = float(cfg.TRAIN.RPN_EACH_BATCH)
        else:
            self._batch = float(batch_str)

        # self._anchors = generate_anchors(base_size=self._feat_stride, scales=np.array([self._scales, self._scales * cfg.ANCHOR_SCALE_OFFSET]),
        #                                  ratios=cfg.GENERATION_ANCHOR_RATIOS)

        self._anchors = generate_anchors(base_size=self._feat_stride, scales=np.array(cfg.TRAIN.MULTI_SCALE_RPN_SCALE[int(self._scales)]),
                                         ratios=cfg.GENERATION_ANCHOR_RATIOS)
    ...
    def forward(self, bottom, top):
        ...
        for i_dim in xrange(self._ndim):
            ...
            labels_gt_inds = np.where(gt_max_overlaps > cfg.TRAIN.GT_MAX_OVERLAPS_THRESH)[0]
            # rpn:all ron:>0.1
            ...
            labels[(max_overlaps < cfg.TRAIN.BG_THRESH_HI) & (max_overlaps >= cfg.TRAIN.BG_THRESH_LO)] = 0
            # rpn:[0, 0.3) ron:[0.001, 0.3)

            if cfg.TRAIN.ALLOW_GT_MAX_OVERLAPS_TO_FG:
        ...
        # rpn:batch:256(fg+bg) ron:fg/bg=1/3(fgs>0)
        ...
        # bg for OHEM
        if cfg.TRAIN.ALLOW_OHEM_TO_BG:
            bg_inds = np.where(all_labels == 0)[0]
            if len(bg_inds) > num_bg:
                anchor_scores = bottom[0].data[:, self._num_anchors:, :, :]
                anchor_scores = anchor_scores.transpose(0, 2, 3, 1).reshape((cfg.TRAIN.BATCHSIZE, K * self._num_anchors)) #.reshape(-1)
                anchor_scores = anchor_scores[:, inds_inside].reshape(-1)
                bg_anchor_scores = anchor_scores[bg_inds]
                bg_low_score_inds = np.argsort(-1 * bg_anchor_scores)[int(num_bg):]
                disable_inds = bg_inds[bg_low_score_inds]
                all_labels[disable_inds] = -1
                # print bg_anchor_scores[np.argsort(-1 * bg_anchor_scores)][:5]
        else:
            bg_inds = np.where(all_labels == 0)[0]
            if len(bg_inds) > num_bg:
        ...
        for i_dim in xrange(self._ndim):
            ...
            bbox_inside_weights = bbox_inside_weights.reshape((1, height, width, self._num_anchors * 4)).transpose(0, 3,
                                                                                                                   1, 2)
            bbox_outside_weights = bbox_outside_weights.reshape((1, height, width, self._num_anchors * 4)).transpose(0,
                                                                                                                     3,
                                                                                                                     1,
                                                                                                                     2)
