...
from utils.blob import prep_im_for_blob, im_list_to_blob, prep_im_for_blob_adapt
...
def get_minibatch(roidb, num_classes):
    ...
    # random crop -> random expension -> color distortion -> resize -> blob generation
    ...
    for i in xrange(num_images):
        ...
        if cfg.TRAIN.MOREAUGMENT:
            ...
            while flag:
                ...
                if reshape_flag > 0:  # keep short x short
                ...
                # plt.imshow(im_crop[:, :, ::-1], aspect='equal')
            ...
            keeps[i] = np.arange(len(roidb[i]['boxes']))
            crop_infos[i] = [0, im.shape[1], 0, im.shape[0]]
            im_crop = im.copy()
        ...
        if cfg.TRAIN.EXPENSION and expension_flag > 0:
            im_expension_size = \
            npr.randint(low=np.max(im_crop.shape[0:2]), high=np.max(im_crop.shape[0:2]) * cfg.TRAIN.EXPENSION_SCALE,
                        size=1)[0]
            ...
            if cfg.TRAIN.EXPENSION_RAND:
                ...
                im_expension = cv2.resize(im_crop, None, None, fx=float(im_expension_size) / im_crop_shape[1], \
                                          fy=float(im_expension_size) / im_crop_shape[0],
                                          interpolation=cv2.INTER_LINEAR)
                im_expension = npr.permutation(im_expension.flatten()).reshape(
                    (im_expension_size, im_expension_size, 3))
            else:
        ...
        if cfg.TRAIN.ADAPT_SCALE:
            im_crop, im_scale = prep_im_for_blob_adapt(im_crop, cfg.PIXEL_MEANS, cfg.TRAIN.MIN_MAX_SCALES[0])
        else:
